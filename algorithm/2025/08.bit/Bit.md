...
| #   | 식                                 | 효과         |
| --- | --------------------------------- | ---------- |
| 1   | `x and (1 shl k)`                 | k번째 비트 검사  |
| 2   | `x or (1 shl k)`                  | k번째 비트 켜기  |
| 3   | `x and (1 shl k).inv()`           | k번째 비트 끄기  |
| 4   | `x xor (1 shl k)`                 | k번째 비트 토글  |
| 5   | `x and -x`                        | 최하위 `1` 추출 |
| 6   | `x and (x - 1)`                   | 최하위 `1` 끄기 |
| 7   | `x.inv() and (x + 1)`             | 최하위 `0` 추출 |
| 8   | `(x and (x - 1)) == 0`            | 2의 거듭제곱 판별 |
| 9   | `(a xor b).countOneBits()`        | 해밍 거리      |
| 10  | `for (mask in 0 until (1 shl n))` | 부분집합 순회    |

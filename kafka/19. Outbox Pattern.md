...
graph TB
    subgraph "Application Layer"
        A[Business Logic] --> B[DB Transaction]
        B --> C[Save Business Data]
        B --> D[Save to Outbox Table]
    end
    
    subgraph "Database"
        C --> E[(Business Tables)]
        D --> F[(Outbox Events)]
        G[Single ACID Transaction] -.-> C
        G -.-> D
    end
    
    subgraph "Publisher Process"
        H[Outbox Publisher] --> I[Poll Outbox Table]
        I --> J{Unpublished Events?}
        J -->|Yes| K[Send to Kafka]
        J -->|No| L[Wait/Sleep]
        L --> I
        K --> M[Mark as Published]
        M --> I
    end
    
    subgraph "Message Broker"
        K --> N[(Kafka Topic)]
        N --> O[Consumer Service 1]
        N --> P[Consumer Service 2]
        N --> Q[Consumer Service 3]
    end
    
    subgraph "Failure Scenarios"
        R[Network Failure] -.-> K
        S[Kafka Broker Down] -.-> N
        T[Publisher Crash] -.-> H
    end
    ...
    classDef outbox fill:#f3e5f5
    classDef publisher fill:#e8f5e8
    classDef failure fill:#ffebee
    
    class B,C,D,G transaction
    class F,I outbox
    class H,K,M publisher
    class R,S,T failure
```
